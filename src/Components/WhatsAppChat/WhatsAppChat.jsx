            ...
            <div className='fixed -right-2 bottom-1 md:bottom-5 md:right-4'>
                ...
                <div onClick={() => setOpenModal(false)} className={`fixed z-[100] w-screen ${openModal ? 'visible opacity-100' : 'invisible opacity-0'} inset-0 grid place-items-center bg-black/20 backdrop-blur-sm duration-100 dark:bg-transparent`}>
                    <div onClick={(e_) => e_.stopPropagation()}
                        className={`absolute right-0 translate-y-1/2 mr-5 max-w-md rounded-lg bg-white p-6 drop-shadow-lg dark:bg-zinc-900 dark:text-white ${openModal ? 'opacity-1 duration-300' : 'scale-110 opacity-0 duration-150'}`}
                    >
                        ...
                        <div className="text-center">
                            <p>Welcome to </p>
                            <h1 className="mb-2 text-2xl font-semibold">H-TutorProvider<sup className="text-green-400">B<span className="text-red-400">D</span></sup></h1>
