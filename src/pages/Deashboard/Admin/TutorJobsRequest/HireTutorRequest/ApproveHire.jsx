...
const ApproveHire = ({ approveHire, refetch, isLoading }) => {
	...
	const handleRowClick = (id) => {
		navigate(`/hire-tutor-apply/details/${id}`);
	};
	if (isLoading) {
		return <Loading />;
	}
					...
					<table className="min-w-[100%] shadow-md border mx-auto border-gray-100">
						...
						<tbody>
							{approveHire.map((hire, inx) => (
								<tr
									key={hire?._id}
									onClick={() => handleRowClick(hire?._id)}
								...
								>
									...
									<td className="py-2">
										<div className="flex items-center text-center justify-center">
											<div>
												<div className="font-medium text-sm">
													{hire?.tutorAddress || "Dhaka, Bangladesh"}
												</div>
												<div className="font-medium text-xs opacity-70">
													{hire?.tutorPhone || "+8801000000000"}
												</div>
											</div>
										</div>
									</td>
									<td className="py-2">
										<div className="flex items-center text-center justify-center">
											<div>
												<div className="font-medium text-sm">
													{hire?.jobSubject || "Sciences"}
												</div>
												<div className="font-medium text-xs opacity-60">
													{hire?.jobCategory || "Home"}
												</div>
											</div>
										</div>
									</td>
									<td className="py-2">
										<div className="flex items-center text-center justify-center">
											<div>
												<div className="font-medium text-sm">
													{hire?.tuitionSalary || "0000"}TK
												</div>
												<div className="font-medium text-xs opacity-60">
													Start {hire?.tuitionStartMonth || "MM"}
												</div>
											</div>
										</div>
									</td>
									<td className="py-2">
										<div className="flex items-center text-center justify-center">
											<div>
												<div className="font-medium text-sm">
													{hire?.parentName || "guardian name"}
												</div>
												<div className="font-medium text-xs opacity-50">
													{hire?.parentId || "P-00000"}
												</div>
											</div>
										</div>
									</td>
									<td className="py-2">
										<div className="flex items-center text-center justify-center">
											<div>
												<div className="font-medium text-sm">
													{hire?.parentAddress || "Dhaka, Bangladesh"}
												</div>
												<div className="font-medium text-xs opacity-70">
													{hire?.parentPhone || "+8801888888888"}
												</div>
											</div>
										</div>
									</td>
									...
									<td className="p-1 text-xs font-medium text-center border-b">
										<button
											onClick={() =>
												handleDeleteById(
													`api/tutor-hire-request/${hire?._id}`,
													"Tutor Hire",
													"Your Tutor Hire"
												)
											}
											className="text-xs px-1 py-2 bg-red-400 hover:bg-red-600 hover:underline text-white rounded-lg transition-all duration-200"
										>
											Cancel
										</button>
									</td>
